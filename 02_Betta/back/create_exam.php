...
for ($i = 0; $i < $length; $i++) {
	...
	//$query = "SELECT * FROM masterquestions WHERE id = '$questionsarr[$i]'";
...
$query="CREATE TABLE IF NOT EXISTS `Exam Status`
    ...
    `exam name` varchar(650) NOT NULL default '',
    `status` int,
	PRIMARY KEY  (`index`)
	) ENGINE = InnoDB;";
...
if($result)
	echo "Exam Status table created.";
else 
	echo "Exams table not created.";

$query="insert into `Exam Status` (`exam name`, `status`) VALUES ('$exam_name', '1')";

$result = $conn->query($query);

if($result)
	echo "Exam status inserted.";
else 
	echo "Exam status inserted.";
